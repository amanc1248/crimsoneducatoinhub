...
import { getAllData, getPaymentsDetails } from "../../actions/homeActions";
import { PaymentFilterP } from "./PaymentFilter.p";

export const PaymentFilterC = () => {
  // data
  const wantedDBList = [
    {
  ...
  const wantedLocalList = ["paymentStatus", "paymentMonth"];
    ...
    for (let i = 0; i < filter.length; i++) {
      ...
      for (let j = 0; j < filter[i].filters.length; j++) {
        if (filter[i].filters[j].checked === true) {
          searchingArray.push(filter[i].filters[j].id);
        }
      }
